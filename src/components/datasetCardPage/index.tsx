...
export default function DatasetCardPageContent() {
  ...
  const shareableLink =
    typeof window !== "undefined" ? `${window.location.origin}${canonicalPath || pathname}` : "";
  ...
  const dataCard: DataCard = dataset;
  const doiHref =
    dataCard.doi &&
    (dataCard.doi.startsWith("http") ? dataCard.doi : `https://doi.org/${dataCard.doi}`);
  const hasAcronym = dataCard.acronym && dataCard.acronym !== "N/A";
          ...
          <div className="p-6 sm:p-8">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm text-gray-600">
                  {hasAcronym && (
                    <span className="inline-flex items-center gap-2 px-3 py-1 bg-white border border-gray-200 rounded-full">
                      ...
                      <span className="font-semibold text-[#24408E]">{dataCard.acronym}</span>
                    </span>
                  )}
                  {doiHref && (
                    <div className="inline-flex items-center gap-1">
                      ...
                      <button
                        onClick={() => copyToClipboard(dataCard.doi!)}
                ...
                <div className="mt-3">
                  <p className="text-xs sm:text-sm text-amber-700">
                    <span className="font-medium">Restricted Access:</span> This dataset is
                    available under restricted access conditions.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 ml-6">
                <div className="flex items-center gap-2 group relative">
                  ...
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                    {dataCard.page_views.toLocaleString()} views
                  </div>
                </div>
                <div className="flex items-center gap-2 group relative">
                  ...
                  <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                    {dataCard.total_downloads.toLocaleString()} downloads
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                ...
                <p className="text-gray-700">{dataCard.study_design}</p>
              </div>

              <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                <h3 className="font-semibold text-gray-900 mb-2">Collection Method</h3>
                <p className="text-gray-700">{dataCard.data_collection_methods}</p>
              </div>

              <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                <h3 className="font-semibold text-gray-900 mb-2">Countries</h3>
                <p className="text-gray-700">{dataCard.countries}</p>
              </div>

              <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                <h3 className="font-semibold text-gray-900 mb-2">Thematic Area</h3>
                <p className="text-gray-700">{dataCard.thematic_area}</p>
              </div>

              <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                <h3 className="font-semibold text-gray-900 mb-2">Version</h3>
                <p className="text-gray-700">{dataCard.version || "1"}</p>
              </div>

              {dataCard.data_format && (
                <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                  <h3 className="font-semibold text-gray-900 mb-2">Data Format</h3>
                  <p className="text-gray-700">{dataCard.data_format}</p>
                </div>
              )}

              {dataCard.protocol_id && (
                <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                  <h3 className="font-semibold text-gray-900 mb-2">Protocol ID</h3>
                  <p className="text-gray-700 break-words">{dataCard.protocol_id}</p>
                </div>
              )}

              {dataCard.country_protocol_id && (
                <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                  <h3 className="font-semibold text-gray-900 mb-2">Country Protocol ID</h3>
                  <p className="text-gray-700 break-words">{dataCard.country_protocol_id}</p>
                </div>
              )}
            </div>

            {dataCard.contacts.length > 0 && (
              <div className="mb-8 pb-8 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">Contact Information</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {dataCard.contacts.map((contact, idx) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              {dataCard.license && (
                <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                  ...
                  <p className="text-gray-700">{dataCard.license}</p>
                </div>
              )}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-gray-200">
              <div className="text-xs sm:text-sm text-gray-500 text-center sm:text-left">
                ...
                <p className="mt-1">Created: {new Date(dataCard.created_at).toLocaleDateString()}</p>
              </div>

              <div className="flex items-center gap-3">
                <Link
                  href={`/datasets/access/${dataCard.id}`}
